...
<?php
	if (isset($_POST['name'])&&isset($_POST['pop'])&&isset($_POST['hob'])&&isset($_POST['food']))
	{
	
	$li = $_POST['name'].', '.$_POST['pop'].', '.$_POST['hob'].', '.$_POST['food']."\n";
	...
	if (file_exists('animals.csv'))
	{
	  ...
	  while (!feof($csvfile))
	  {
	    ...
	    if (!empty($line))
	    {
	      if ($ctr==0)
	      {
		echo '<tr><th>'.$line[0].'</th><th>'.$line[1].'</th><th>'.$line[2].'</th><th>'.$line[3].'</th></tr>';
		$ctr++;
	      }//end categories
	      else
	      {
...
<tr>
 ...
 <td> <input name="food" size="10"/></td></tr></table>
